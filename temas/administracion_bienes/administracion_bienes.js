document.addEventListener('DOMContentLoaded', () => {
    ...
    const obtenerPreguntas = async () => {
        try {
            const response = await fetch('preguntas_administracion_bienes.json');
            if (!response.ok) {
    ...
    const guardarIntento = (puntaje) => {
        ...
        localStorage.setItem('simulacro_administracion_bienes', JSON.stringify(intentos));
    };

    const obtenerIntentos = () => {
        const intentosJSON = localStorage.getItem('simulacro_administracion_bienes');
