document.addEventListener('DOMContentLoaded', () => {
    ...
    const obtenerPreguntas = async () => {
        try {
            const response = await fetch('preguntas_gestion_publica.json');
            if (!response.ok) {
    ...
    // Función para seleccionar 20 preguntas aleatorias de las 70
    ...
    const finalizarSimulacro = () => {
        ...
        resultadosContainer.innerHTML = `
            ...
            <p class="nota">Recuerda: El puntaje mínimo para pasar la prueba funcional es de 65.00 puntos[cite: 108, 109, 110].</p>
            <br>
    ...
    const guardarIntento = (puntaje) => {
        ...
        localStorage.setItem('simulacro_gestion_publica', JSON.stringify(intentos));
    };

    const obtenerIntentos = () => {
        const intentosJSON = localStorage.getItem('simulacro_gestion_publica');
