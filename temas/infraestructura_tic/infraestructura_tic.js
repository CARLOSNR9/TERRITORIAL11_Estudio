document.addEventListener('DOMContentLoaded', () => {
    ...
    const obtenerPreguntas = async () => {
        try {
            const response = await fetch('preguntas_infraestructura_tic.json');
            if (!response.ok) {
    ...
    const guardarIntento = (puntaje) => {
        ...
        localStorage.setItem('simulacro_infraestructura_tic', JSON.stringify(intentos));
    };

    const obtenerIntentos = () => {
        const intentosJSON = localStorage.getItem('simulacro_infraestructura_tic');
