document.addEventListener('DOMContentLoaded', () => {
    ...
    const obtenerPreguntas = async () => {
        try {
            const response = await fetch('preguntas_normatividad_tic.json');
            if (!response.ok) {
    ...
    const guardarIntento = (puntaje) => {
        ...
        localStorage.setItem('simulacro_normatividad_tic', JSON.stringify(intentos));
    };

    const obtenerIntentos = () => {
        const intentosJSON = localStorage.getItem('simulacro_normatividad_tic');
