document.addEventListener('DOMContentLoaded', () => {
    ...
    const obtenerPreguntas = async () => {
        try {
            const response = await fetch('preguntas_razonamiento_analitico.json');
            if (!response.ok) {
    ...
    const guardarIntento = (puntaje) => {
        ...
        localStorage.setItem('simulacro_razonamiento_analitico', JSON.stringify(intentos));
    };

    const obtenerIntentos = () => {
        const intentosJSON = localStorage.getItem('simulacro_razonamiento_analitico');
