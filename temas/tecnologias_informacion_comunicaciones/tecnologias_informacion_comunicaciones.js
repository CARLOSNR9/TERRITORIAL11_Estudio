document.addEventListener('DOMContentLoaded', () => {
    ...
    const obtenerPreguntas = async () => {
        try {
            const response = await fetch('preguntas_tecnologias_informacion_comunicaciones.json');
            if (!response.ok) {
    ...
    const guardarIntento = (puntaje) => {
        ...
        localStorage.setItem('simulacro_tecnologias_informacion_comunicaciones', JSON.stringify(intentos));
    };

    const obtenerIntentos = () => {
        const intentosJSON = localStorage.getItem('simulacro_tecnologias_informacion_comunicaciones');
